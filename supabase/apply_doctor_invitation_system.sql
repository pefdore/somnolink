...
BEGIN
    ...
    IF v_patient_id IS NOT NULL THEN
        ...
        ELSE
            ...
            RETURN QUERY SELECT v_patient_id, 'linked', 'Patient existant associé avec succès';
        END IF;
    ELSE
        ...
        -- Associer immédiatement
        INSERT INTO patient_doctor_relationships 
        (patient_id, doctor_id, status, association_type, invited_by, invited_at, accepted_at)
        VALUES (v_patient_id, v_doctor_id, 'active', 'invitation', v_doctor_id, NOW(), NOW());

        RETURN QUERY SELECT v_patient_id, 'created_linked', 'Nouveau patient créé et associé';
    END IF;
END;
$$;
