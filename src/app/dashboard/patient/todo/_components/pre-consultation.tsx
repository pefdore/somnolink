...
const formSchema = z.object({
  ...
  pichot: z.record(z.string(), z.string()).default({}),
  stopbang: z.record(z.string(), z.string()).default({}),
...
export function PreConsultation({ patientId, appointment, isDone }: { patientId: string, appointment: Appointment, isDone: boolean }) {
  ...
  async function onSubmit(values: FormValues) {
    ...
    try {
      ...
      // 2. Création/Mise à jour du rendez-vous (si nécessaire)
      console.log('Vérification du rendez-vous existant');
      let appointmentData = appointment;
      if (!appointment) {
        // Si pas de rendez-vous fourni, on ne crée pas de nouveau rendez-vous
        console.log('Aucun rendez-vous fourni, utilisation des données existantes');
      }
      if (!appointmentData) {
        console.error('Aucun rendez-vous disponible pour soumettre le questionnaire');
        throw new Error('Aucun rendez-vous trouvé pour soumettre le questionnaire');
      }
      console.log('Rendez-vous disponible:', appointmentData.id);
  ...
  console.log('Rendering pre-consultation component, currentStep:', currentStep, 'totalSteps:', totalSteps);

  return (
    <div className="p-4 md:p-8 bg-white rounded-lg shadow-2xl max-w-4xl mx-auto">
      <div className="text-center mb-8">
        <h2 className="text-2xl font-bold text-gray-800">ÉVALUATION DES TROUBLES DU SOMMEIL</h2>
        <p className="text-sm text-gray-600 mt-2">Étape {currentStep + 1} sur {totalSteps}</p>
        <div className="w-full bg-gray-200 rounded-full h-2.5 mt-4">
          <div className="bg-blue-600 h-2.5 rounded-full" style={{ width: `${((currentStep + 1) / totalSteps) * 100}%` }}></div>
        </div>
      </div>
        <Form {...form}>
            <form onSubmit={form.handleSubmit(onSubmit, (errors) => {
                ...
                <div className="mt-10 flex justify-between">
                    ...
                    {currentStep < totalSteps - 1 ? (<Button type="button" onClick={handleNext} disabled={currentStep === 2 && !isEpworthLocked}>Suivant</Button>) : (<Button type="submit" disabled={isSubmitting}> {isSubmitting ? "Enregistrement..." : "Enregistrer et Soumettre"}</Button>)}
                </div>
