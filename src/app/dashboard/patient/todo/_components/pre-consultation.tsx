...
const formSchema = z.object({
  ...
  epworth: z.record(z.string(), z.string().min(1, "Requis")),
  ...
  medical_history: z.record(z.string(), z.boolean().default(false)).optional().default({}),
  medical_history_other: z.string().optional().default(''),
  current_treatments: z.string().optional().default(''),
  life_habits: z.record(z.string(), z.any()).optional().default({}),
  driving_info: z.record(z.string(), z.any()).optional().default({}),
  sleep_disorder_history: z.record(z.string(), z.any()).optional().default({}),
  free_comments: z.string().optional().default(''),
  consent_signature: z.boolean().optional().default(false),
}).superRefine((data, ctx) => {
    const epworthScores = Object.values(data.epworth || {}) as string[];
    const totalEpworthScore = epworthScores.reduce((sum: number, value: string) => sum + (parseInt(value, 10) || 0), 0);
...
export function PreConsultation({ patientId, appointment, isDone }: { patientId: string, appointment: Appointment, isDone: boolean }) {
  ...
  const form = useForm<FormValues>({ resolver: zodResolver(formSchema), mode: "onTouched", defaultValues: { filling_date: new Date().toISOString().split('T')[0], epworth: {}, consent_signature: false } });
  ...
  const handleNext = async () => { const fieldsToValidate = stepFields[currentStep] as (keyof FormValues)[]; const isValid = await trigger(fieldsToValidate); if (isValid) { setCurrentStep(prev => prev + 1); window.scrollTo(0, 0); } };
  const handlePrevious = () => { setCurrentStep(prev => prev - 1); window.scrollTo(0, 0); };
  const handleLockEpworth = async () => { const epworthFields = Object.keys(form.getValues().epworth || {}).map(k => `epworth.${k}`) as (keyof FormValues)[]; const isValid = await trigger(epworthFields); if (isValid) { setIsEpworthLocked(true); } };
